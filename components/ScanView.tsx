...
export const ScanView = () => {
  const { setView, addTransaction, homeCurrency, rates, initialInputMode, setInitialInputMode, selectedMonth } = useApp();
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        const result = await parseReceiptImage(base64Data, 'image/jpeg'); // Always jpeg after resize
  ...
  const handleSave = () => {
    ...
    // Check if date is in current selected month
    const tDate = parseDate(date);
    const isSameMonth = tDate.getMonth() === selectedMonth.getMonth() && tDate.getFullYear() === selectedMonth.getFullYear();

    if (!isSameMonth) {
      const monthName = tDate.toLocaleString('default', { month: 'long', year: 'numeric' });
      alert(`Transaction saved to ${monthName}. Switch to that month in Dashboard to view it.`);
    }
  ...
  return (
    <div className="h-full flex flex-col max-w-2xl mx-auto p-4">
      ...
      {mode === 'manual' && (
        <>
          <div className="flex-1 overflow-y-auto pb-32 animate-slide-up">
          ...
          <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-50 md:relative md:shadow-none md:border-0 md:bg-transparent md:p-0 md:mt-8">
            <button
              onClick={handleSave}
